"""Seed baseline Repo nodes into the target database."""
...
def seed(cfg: DbConfig, log: structlog.BoundLogger) -> None:
    with GraphDatabase.driver(cfg.uri, auth=(cfg.user, cfg.password)) as driver:  # pyright: ignore[reportUnknownMemberType]
        with driver.session(database=cfg.database) as s:  # pyright: ignore[reportUnknownMemberType]
            result = s.run(
                "UNWIND $urls AS url MERGE (r:Repo {github_url: url}) RETURN count(r) AS total",
                urls=_REPOS,
            )
            row = result.single()
            total = int(row["total"]) if row else 0
        log.info("repos_seeded", count=total, database=cfg.database)
